...
class WC_Emails {
	 ...
	 */
	public function email_header( $email_heading,$order_id ) {
		wc_get_template( 'emails/email-header.php', array( 'email_heading' => $email_heading ,'order_id'=> $order_id) );
	}
	...
	public function order_meta( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if ( $order->customer_note ) {
			$fields['customer_note'] = array(
				'label' => __( 'Anmerkungen', 'woocommerce' ),
			...
			if ( $plain_text ) {
				...
				foreach ( $fields as $field ) {
					if ( isset( $field['label'] ) && isset( $field['value'] ) && $field['value'] ) {
						...
						echo '<p><span>' . $field['value'] . '</span></p>';
					}
				}
			}
		}
	 ...
	 * @return string
	 */
	public function customer_details( $order, $sent_to_orderadmin = false, $plain_text = false ) {
		$fields = array();
	
		if ( $order->billing_email ) {
			$fields['billing_email'] = array(
				'label' => __( '', 'woocommerce' ),
				'value' => wptexturize( '<a class="email_address">'.$order->billing_email.'</a>')
			);
	    }
