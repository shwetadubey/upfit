var TidioChatWP = {
    ...
    getProjects: function (access_key) {
        jQuery.get("//www.tidiochat.com/api/v3/project?accessKey=" + access_key, function (data) {
            TidioChatWP.renderProjects(data);
        }, 'json');
    },
    ...
    init: function () {
        ...
        login_button.click(function (e) {
            ...
            jQuery.get("//www.tidiochat.com/api/v3/user/access?email=" + email + "&password=" + password, function (data) {
                if (data.status == true && data.value != "ERR_DATA_INVALID") {
                    TidioChatWP.access_key = data.value;
                    TidioChatWP.getProjects(data.value);
                } else {
