...
if ( ! class_exists( 'YITH_Report_Vendors_Sales' ) ) {
    ...
    class YITH_Report_Vendors_Sales extends WC_Admin_Report {
        ...
        public function get_chart_legend() {
            ...
            $legend[] = array(
                'title' => sprintf( __( '%s sales for the selected vendor', 'yith_wc_product_vendors' ), '<strong>' . wc_price( $report_data[ $vendor_id ]['sales'] ) . '</strong>' ),
                'color' => $this->chart_colours['vendor_amount'],
                'highlight_series' => 0
            );

            $legend[] = array(
                'title' => sprintf( __( '%s purchases for the selected vendor', 'yith_wc_product_vendors' ), '<strong>' . $report_data[ $vendor_id ]['items_number'] . '</strong>' ),
        ...
        public function get_chart_widgets() {
            ...
            if ( ! empty( $this->vendor_ids ) ) {
                $widgets[] = array(
                    'title'    => __( 'Showing reports for:', 'yith_wc_product_vendors' ),
        ...
        public function vendors_widget() {
            ...
            ?>
            <h4 class="section_title"><span><?php _e( 'Search Vendors', 'yith_wc_product_vendors' ); ?></span></h4>
            <div class="section">
                <form method="GET">
                    <div>
                        <input type="hidden" class="wc-product-search" style="width:203px;" name="vendor_ids" data-placeholder="<?php _e( 'Search for a vendor&hellip;', 'yith_wc_product_vendors' ); ?>" data-action="yith_json_search_vendors" />
                        <input type="submit" class="submit button" value="<?php _e( 'Show', 'yith_wc_product_vendors' ); ?>" />
            ...
            <h4 class="section_title"><span><?php _e( 'Top Sellers', 'yith_wc_product_vendors' ); ?></span></h4>
            <div class="section">
                <table cellspacing="0">
                    <?php
                    ...
                    if ( $top_sellers ) {
                        ...
                        echo '<tr><td colspan="3">' . __( 'No vendors found in range', 'yith_wc_product_vendors' ) . '</td></tr>';
                    }
                    ?>
                </table>
            </div>
            <h4 class="section_title"><span><?php _e( 'Top Earners', 'yith_wc_product_vendors' ); ?></span></h4>
            <div class="section">
                <table cellspacing="0">
                    <?php
        ...
        public function get_main_chart() {
            ...
            if ( ! $this->vendor_ids ) {
                ...
                <div class="chart-container">
                    <p class="chart-prompt"><?php _e( '&larr; Choose a vendor to view stats', 'yith_wc_product_vendors' ); ?></p>
                </div>
                <?php
                ...
                <script type="text/javascript">
                    ...
                    jQuery(function(){
                        ...
                        var drawGraph = function( highlight ) {
                            ...
                            var series = [
                                {
                                    label: "<?php echo esc_js( __( 'Number of items sold', 'yith_wc_product_vendors' ) ) ?>",
                                ...
                                {
                                    label: "<?php echo esc_js( __( 'Sales amount', 'yith_wc_product_vendors' ) ) ?>",
