...
if ( ! class_exists( 'YITH_Report_Vendors_Registered' ) ) {
    ...
    class YITH_Report_Vendors_Registered extends WC_Admin_Report {
        ...
        public function get_chart_legend() {
            ...
            $legend = array(
                'totals' => array(
                    'title'            => sprintf( __( '%s registered vendors', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['totals'] . '</strong>' ),
                    'title'            => sprintf( __( '%s registered vendors', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['totals'] . '</strong>' ),
                ...
                'enabled' => array(
                    'title'            => sprintf( __( '%s with selling capability', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['enabled'] . '</strong>' ),
                    'widget_title'     => sprintf( __( '%s enabled', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['enabled'] . '</strong>' ),
                    'color'            => $this->chart_colours['enabled'],
                    'highlight_series' => 1
                ),

                'disabled' => array(
                    'title'            => sprintf( __( '%s without selling capability', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['disabled'] . '</strong>' ),
                    'widget_title'     => sprintf( __( '%s disabled', 'yith_wc_product_vendors' ), '<strong>' . $this->chart_data['disabled'] . '</strong>' ),
            ...
            <script type="text/javascript">
                ...
                jQuery(function(){
                    ...
                    var drawGraph = function( highlight ) {
                        var series = [
                            {
                                label: "<?php echo esc_js( __( 'Totals of registered vendors', 'yith_wc_product_vendors' ) ) ?>",
                                data: plot_data,
                                yaxis: 1,
                                ...
                                enable_tooltip: true,
								append_tooltip: "<?php echo ' ' . __( 'new vendors', 'yith_wc_product_vendors' ); ?>",
                                shadowSize: 0,
                                hoverable: true
                            }
                        ];
        ...
        public function enabled_vs_disabled() {
            ...
            ?>
            <div class="chart-container">
                <h3 style="text-align: center"><?php _e( 'Enabled Vs Disabled Vendors', 'yith_wc_product_vendors' ) ?></h3>
                    ...
                    jQuery.plot(
                        ...
                        ],
                        {
                            ...
                            series: {
                                ...
                                enable_tooltip: true,
                                append_tooltip: "<?php echo ' ' . __( 'vendors', 'yith_wc_product_vendors' ); ?>"
                            },
                            legend: {
                                show: false
                            }
