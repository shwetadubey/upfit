<?php 
/*
 * Template Name: PDF9 (Nutrition plan of saturday)
 */
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <body class="grey">
	...
	$sat_meal_res=$wpdb->get_results('select sat from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id);	
	$sat_meal=unserialize($sat_meal_res[0]->sat);
	//print_r($mon_meal);
	
	if(!empty($sat_meal['breakfast']))
		$breakfast=array_values($sat_meal['breakfast']);
	if(!empty($sat_meal['lunch']))
		$lunch=array_values($sat_meal['lunch']);
	if(!empty($sat_meal['dinner']))
		$dinner=array_values($sat_meal['dinner']);
	if(!empty($sat_meal['pre_dinner_snack']))
		$pre_dinner_snack=array_values($sat_meal['pre_dinner_snack']);
      ...
      <div style="background:#f0eeef;margin:0 auto;padding:0px 0px;display:inline-block;outline:none; line-height:22px;height:100%;">
		...
		// echo $order_details[0]->meals_per_day;
		  if($order_details[0]->meals_per_day==5){
			if(!empty($sat_meal['pre_lunch_snack'])) 
				$pre_lunch_snack=array_values($sat_meal['pre_lunch_snack']);
