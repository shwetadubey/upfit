<?php 
/*
 * Template Name: PDF7 (Nutrition plan of thursday)
 */
?>
<!DOCTYPE html>
<html lang="en">
...
//	  print_r($item_data);
	...
	$thu_meal_res=$wpdb->get_results('select thu from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id);	
	$thu_meal=unserialize($thu_meal_res[0]->thu);
	//print_r($mon_meal);
	
	if(!empty($thu_meal['breakfast']))
		$breakfast=array_values($thu_meal['breakfast']);
	if(!empty($thu_meal['lunch']))
		$lunch=array_values($thu_meal['lunch']);
	if(!empty($thu_meal['dinner']))
		$dinner=array_values($thu_meal['dinner']);
	if(!empty($thu_meal['pre_dinner_snack']))
		$pre_dinner_snack=array_values($thu_meal['pre_dinner_snack']);
      ...
      <div style="background:#f0eeef;margin:0 auto;padding:0px 0px;display:inline-block;outline:none; line-height:22px;height:100%;">
		...
		// echo $order_details[0]->meals_per_day;
		  if($order_details[0]->meals_per_day==5){
			 	if(!empty($thu_meal['pre_lunch_snack']))
				$pre_lunch_snack=array_values($thu_meal['pre_lunch_snack']);
