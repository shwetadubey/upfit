<?php 
/*
 * Template Name: PDF8 (Nutrition plan of friday)
 */
?>
<!DOCTYPE html>
<html lang="en">
...
//	  print_r($item_data);
	...
	$fri_meal_res=$wpdb->get_results('select fri from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id);	
	$fri_meal=unserialize($fri_meal_res[0]->fri);
	//print_r($mon_meal);
	
	if(!empty($fri_meal['breakfast']))
		$breakfast=array_values($fri_meal['breakfast']);
	if(!empty($fri_meal['lunch']))
		$lunch=array_values($fri_meal['lunch']);
	if(!empty($fri_meal['dinner']))
		$dinner=array_values($fri_meal['dinner']);
	if(!empty($fri_meal['pre_dinner_snack']))
		$pre_dinner_snack=array_values($fri_meal['pre_dinner_snack']);
		...
		// echo $order_details[0]->meals_per_day;
		 if($order_details[0]->meals_per_day==5){
			if(!empty($tue_meal['pre_lunch_snack']))	 
				$pre_lunch_snack=array_values($fri_meal['pre_lunch_snack']);
