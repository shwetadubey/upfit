<?php 
/*
 * Template Name: PDF4 (Nutrition plan of monday)
 */
?>
<!DOCTYPE html>
<html lang="en">
...
//	  phpinfo();
	...
	//echo 'select id,meals_per_day from up_user_nutrition_plans where order_id='.$order_id.' AND site_id='.$site_id
	...
	//echo 'select mon from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id;
	$mon_meal_res=$wpdb->get_results('select mon from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id);	
	
	$lunch=$breakfast=$dinner=$pre_dinner_snack='';
	$mon_meal=unserialize($mon_meal_res[0]->mon);
	if(!empty($mon_meal['breakfast']))
		$breakfast=array_values($mon_meal['breakfast']);
	if(!empty($mon_meal['lunch']))
		$lunch=array_values($mon_meal['lunch']);
	if(!empty($mon_meal['dinner']))
		$dinner=array_values($mon_meal['dinner']);
	if(!empty($mon_meal['pre_dinner_snack']))
		$pre_dinner_snack=array_values($mon_meal['pre_dinner_snack']);
	...
	//--------------prioritise_val---------//
//	echo "CALL get_kcal_of_order_meals('".$order_id."','".$breakfast[0]."')";
...
//	print_r($breakfast_kcal);
	...
	foreach($meat_meals2 as $k=>$v){
	  ...
	  ?>
   <div style="background:#f0eeef;position: relative;overflow: hidden;text-align:center;overflow:hidden; color:#70848e;marign:0">
      <div style="padding:0px;display:inline-block;outline:none;margin:0; line-height:22px; ">
          	
          <div style="width:93.18%;text-align:center; margin:0 36px 0 38.4px;  height:94%;">
         <!-- 1 box -->
            <div class="p4-listStyleP" style="width: 24.99%; float:left; ">
				<?php 
				if(!empty($breakfast[0])){ 
		...
		// echo $order_details[0]->meals_per_day;
			if($order_details[0]->meals_per_day==5){
				if(!empty($mon_meal['pre_lunch_snack'])){
					$pre_lunch_snack=array_values($mon_meal['pre_lunch_snack']);
				}
