...
$res=$om_id=$f_ids=array();
	...
	foreach($pre_lunch_snack_ingredients as $bi){
		$f_ids[]=$bi['f_id'];
	}
	//print_r($f_ids);
	...
	$time_mins = $str_time;
			...
			$meat_meals1 = $wpdb->get_results('call meat_and_fish_meals("' . $pre_lunch_snack[0] . '","fleisch")', OBJECT_K);
...
?>
				<div  style="background:#FFF;padding:10px 16px;margin:15px 0px 0px 15px;border-radius:5px;">
					<!-----------pre_lunch_snack-------------->
						...
						else if($meat_meals1[$pre_lunch_snack[0]]->fleisch > 0 || $meat_meals3['fleischwaren'] > 0){
						?>
							<td width="48px">
						...
						else if($fisch_meals1[$pre_lunch_snack[0]]->fisch > 0 ||  $fisch_meals2[$pre_lunch_snack[0]]->Schalentiere > 0){
						?>
							<td width="48px">
