<?php 
/*
 * Template Name: PDF6 (Nutrition plan of wednesday)
 */
?>
<!DOCTYPE html>
<html lang="en">
...
//	  print_r($item_data);
	...
	$wed_meal_res=$wpdb->get_results('select wed from up_plan_logs where user_nutrition_plan_id='.$order_details[0]->id.' AND week_no='.$week_no.' AND site_id='.$site_id);	
	$wed_meal=unserialize($wed_meal_res[0]->wed);
	
	//print_r($mon_meal);
	if(!empty($wed_meal['breakfast']))
		$breakfast=array_values($wed_meal['breakfast']);
	if(!empty($wed_meal['lunch']))
		$lunch=array_values($wed_meal['lunch']);
	if(!empty($wed_meal['dinner']))
		$dinner=array_values($wed_meal['dinner']);
	if(!empty($wed_meal['pre_dinner_snack']))
		$pre_dinner_snack=array_values($wed_meal['pre_dinner_snack']);
...
//	print_r($breakfast_kcal);
    ...
    <div style="background:#f0eeef;text-align:center;overflow:hidden; color:#70848e;">
      <div style="margin:0 auto;padding:0px 0px;display:inline-block;outline:none; line-height:22px;height:100%;">
		...
		// echo $order_details[0]->meals_per_day;
		  if($order_details[0]->meals_per_day==5){
			 	if(!empty($wed_meal['pre_lunch_snack']))
				$pre_lunch_snack=array_values($wed_meal['pre_lunch_snack']);
