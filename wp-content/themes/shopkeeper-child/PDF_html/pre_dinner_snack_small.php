<?php
	...
	$pre_dinner_snack_details= $wpdb->get_results('select id,name from up_meals where id='.$pre_dinner_snack[0]);
	$query='select uf.id as f_id,umi.* from up_meal_ingredients umi join up_foods uf on uf.name=umi.name where umi.meal_id='.$pre_dinner_snack[0];	
	$pre_dinner_snack_ingredients=$wpdb->get_results( $query,ARRAY_A);
	foreach($pre_dinner_snack_ingredients as $bi){
		$f_ids[]=$bi['f_id'];
	}
	//print_r($f_ids);
	...
	if($pre_dinner_snack_order_meals[0]['exchangble']==1){
		$res=array_diff($pre_dinner_snack_final_ingredients,$f_ids);
		$e_id=array_values($res);
	}
if(empty($e_id) && count($e_id) <= 0){
			$res2=array_diff($f_ids,$pre_dinner_snack_final_ingredients);
			$om_id=array_values($res2);
		}
	...
	$time_mins=$str_time;
			$meat_meals1 = $wpdb->get_results('call meat_and_fish_meals("' . $pre_dinner_snack[0] . '","fleisch")', OBJECT_K);
		$meat_meals2 = $wpdb->get_results('call meat_and_fish_meals("' . $pre_dinner_snack[0] . '","Fleischwaren & Wurstwaren")', OBJECT_K);
		$fisch_meals1 = $wpdb->get_results('call meat_and_fish_meals("' . $pre_dinner_snack[0] . '","fisch")', OBJECT_K);
...
?>
	<div style="background:#FFF;padding:10px 16px 5px;margin:15px 0px 0px 15px;border-radius:5px;">
		
		<div style="border-bottom:0.0315em solid #bec8cc; text-align:center; padding-bottom:17.4px;  margin-bottom:17.4px;">
		 <!-----------pre_dinner_snack-------------->
		
		  <h2 class="p4_meal_heading">NACHMITTAGSSNACK</h2>
		  <p style="font-size:12px;margin:3px 0px 8px;"><?php echo $pre_dinner_snack_details[0]->name;?></p>
		  <table class="p4-upfiticon">
			<tr>
				...
				if($meal_extra_details[0]['is_paleo']==1){
					...
					else if($meat_meals1[$pre_dinner_snack[0]]->fleisch > 0 || $meat_meals3['fleischwaren'] > 0){
					?>
						<td width="48px">
					...
					else if($fisch_meals1[$pre_dinner_snack[0]]->fisch > 0 ||  $fisch_meals2[$pre_dinner_snack[0]]->Schalentiere > 0){
					?>
						<td width="48px">
	   ...
	   <div style="border-bottom:0.0315em solid #bec8cc; text-align:left; margin-bottom:14px;  padding-bottom:10.2px;">					  
		  <table class="p4-listingingredients" width="100%" cellspacing="0" cellpadding="0" border="0" style="border:0;outline:none; padding-bottom:0px;">
			<?php 
				$c=$e=0;
				unset($prise_ar);
				$prise_ar=array();
	   ...
	   <div class="p4-listStyleP">
			  ...
			  <p class="letterspacing_5" style="margin: 0;"><?php echo $meal_details[0]['instruction']; ?></p>
	   </div>
	</div>
