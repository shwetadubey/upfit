...
$sweet_tooth= $_GET['sweet_tooth']?$_GET['sweet_tooth']:'';
$is_time_to_cook= $_GET['perpare_time']?$_GET['perpare_time']:'much';
$where_food_buy= $_GET['food']?$_GET['food']:'premium';
...
/* function incl_meal($days_npes_cal,$kcal, $protein, $fatt, $carbo) {
  $temp_array = array();
  $pro_diff = protein_cal($protein);
  $fatt_diff = fatt_cal($fatt);
  $carbo_diff = carbo_cal($carbo);

  foreach ($days_npes_cal as $day => $d) {
  // check kcal in beetween of -15% and +15% value
  if ($kcal >= $d[$day]['breakfast'][1] && $kcal <= $d[$day]['breakfast'][2]) {

  $kcal_diff = abs(((($kcal / $d[$day]['breakfast'][0]) * 100) - 100) * 1.5);
  $diff = $kcal_diff + $pro_diff + $fatt_diff + $carbo_diff;

  $temp_array[$day]['breakfast'] = $diff;
  }
  if (isset($d[$day]['pre_lunch_snack'])) {
  if ($kcal >= $d[$day]['pre_lunch_snack'][1] && $kcal <= $d[$day]['pre_lunch_snack'][2]) {

  $kcal_diff = abs(((($kcal / $d[$day]['pre_lunch_snack'][0]) * 100) - 100) * 1.5);
  $diff = $kcal_diff + $pro_diff + $fatt_diff + $carbo_diff;

  $temp_array[$day]['pre_lunch_snack'] = $diff;
  }
  }


  if ($kcal >= $d[$day]['lunch'][1] && $kcal <= $d[$day]['lunch'][2]) {

  $kcal_diff = abs(((($kcal / $d[$day]['lunch'][0]) * 100) - 100) * 1.5);
  $diff = $kcal_diff + $pro_diff + $fatt_diff + $carbo_diff;

  $temp_array[$day]['lunch'] = $diff;
  }

  if ($kcal >= $d[$day]['pre_dinner_snack'][1] && $kcal <= $d[$day]['pre_dinner_snack'][2]) {

  $kcal_diff = abs(((($kcal / $d[$day]['pre_dinner_snack'][0]) * 100) - 100) * 1.5);
  $diff = $kcal_diff + $pro_diff + $fatt_diff + $carbo_diff;

  $temp_array[$day]['pre_dinner_snack'] = $diff;
  }

  if ($kcal >= $d[$day]['dinner'][1] && $kcal <= $d[$day]['dinner'][2]) {

  $kcal_diff = abs(((($kcal / $d[$day]['dinner'][0]) * 100) - 100) * 1.5);
  $diff = $kcal_diff + $pro_diff + $fatt_diff + $carbo_diff;

  $temp_array[$day]['dinner'] = $diff;
  }
  }
  ...
  function kcal_cal($days_npes_cal, $kcal) {
  //        foreach ($days_npes_cal as $d) {
  //
  //        }
  }

  function protein_cal($protein_val, $ideal_val = 40) {
  $temp_val = abs((($ideal_val / $protein_val) * 100) - 100);

  return $temp_val;
  }

  function fatt_cal($fat_val, $ideal_val = 20) {
  $temp_val = abs((($ideal_val / $fat_val) * 100) - 100);

  return $temp_val;
  }

  function carbo_cal($carbo_val, $ideal_val = 40) {
  $temp_val = abs((($ideal_val / $carbo_val) * 100) - 100);
...
function select_balance_meals($rem_meals = array(), $variance, $micros_array, $days_npes_cal, $per_day_no_of_melas = 4, $protin, $fat, $carbo, $i = 0, $fmeals = array()) {
                ...
                foreach ($variance as $k => $m) {
                        ...
                        foreach ($days_npes_cal as $d => $v) {
                                ...
                                if ($kcal >= $v['breakfast'][1] && $kcal <= $v['breakfast'][2]) {
                                        ...
                                        $micros_diff[$d]['b'][$micros_array[$k]['id']] = $diff;
                                ...
                                if ($breakfast > 0) {
                                        ...
                                        if ($kcal >= $v['lunch'][1] && $kcal <= $v['lunch'][2]) {
                                                ...
                                                $micros_diff[$d]['l'][$micros_array[$k]['id']] = $diff;
                                ...
                                if ($lunch > 0) {
                                        if ($kcal >= $v['dinner'][1] && $kcal <= $v['dinner'][2]) {
                                                ...
                                                $micros_diff[$d]['d'][$micros_array[$k]['id']] = $diff;
                                ...
                                if ($dinner > 0) {
                                        ...
                                        if ($kcal >= $v['pre_dinner_snack'][1] && $kcal <= $v['pre_dinner_snack'][2]) {
                                                ...
                                                $micros_diff[$d]['s'][$micros_array[$k]['id']] = $diff;
                                ...
                                if ($snack > 0 && isset($v['pre_lunch_snack'])) {
                                        if ($kcal >= $v['pre_lunch_snack'][1] && $kcal <= $v['pre_lunch_snack'][2]) {
                                                ...
                                                $micros_diff[$d]['s1'][$micros_array[$k]['id']] = $diff;
                ...
                foreach ($micros_array as $m) {
                        if (empty($rem_meals)) {
                                if (!in_array($m['id'], $meals)) {
                                        ...
                                        $sum = abs(((2 * $protin) - $micros_array[$meals['breakfast']]['Protein']) - $m['Protein']) + abs(((2 * $fat) - $micros_array[$meals['breakfast']]['Fett'] - $m['Fett'])) + abs(((2 * $carbo) - $micros_array[$meals['breakfast']]['Kohlenhydrate'] - $m['Kohlenhydrate']));
                                        $micros_diff[$m['id']] = $sum;
                                }
                        } else {
                                if (in_array($m['id'], $rem_meals)) {
                                        if (!in_array($m['id'], $meals)) {
                                                $sum = abs(((2 * $protin) - $micros_array[$meals['breakfast']]['Protein']) - $m['Protein']) + abs(((2 * $fat) - $micros_array[$meals['breakfast']]['Fett'] - $m['Fett'])) + abs(((2 * $carbo) - $micros_array[$meals['breakfast']]['Kohlenhydrate'] - $m['Kohlenhydrate']));
                                                $micros_diff[$m['id']] = $sum;
                                        }
                ...
                foreach ($micros_array as $m) {
                        if (empty($rem_meals)) {
                                if (!in_array($m['id'], $meals)) {
                                        ...
                                        $sum = abs(((2 * $protin) - $micros_array[$meals['lunch']]['Protein']) - $m['Protein']) + abs(((2 * $fat) - $micros_array[$meals['lunch']]['Fett'] - $m['Fett'])) + abs(((2 * $carbo) - $micros_array[$meals['lunch']]['Kohlenhydrate'] - $m['Kohlenhydrate']));
                                        $micros_diff[$m['id']] = $sum;
                                }
                        } else {
                                if (in_array($m['id'], $rem_meals)) {
                ...
                foreach ($micros_array as $m) {
                        if (empty($rem_meals)) {
                                if (!in_array($m['id'], $meals)) {
                                        ...
                                        $sum = abs(((2 * $protin) - $micros_array[$meals['dinner']]['Protein']) - $m['Protein']) + abs(((2 * $fat) - $micros_array[$meals['dinner']]['Fett'] - $m['Fett'])) + abs(((2 * $carbo) - $micros_array[$meals['dinner']]['Kohlenhydrate'] - $m['Kohlenhydrate']));
                                        $micros_diff[$m['id']] = $sum;
                                }
                        } else {
                                if (in_array($m['id'], $rem_meals)) {
        ...
        if ($per_day_no_of_melas == 5) {
                if (!empty($micros_diff)) {
                        ...
                        foreach ($micros_array as $m) {
                                if (empty($rem_meals)) {
                                        if (!in_array($m['id'], $meals)) {
                                                ...
                                                $sum = abs(((2 * $protin) - $micros_array[$meals['snack']]['Protein']) - $m['Protein']) + abs(((2 * $fat) - $micros_array[$meals['snack']]['Fett'] - $m['Fett'])) + abs(((2 * $carbo) - $micros_array[$meals['snack']]['Kohlenhydrate'] - $m['Kohlenhydrate']));
                                                $micros_diff[$m['id']] = $sum;
                                        }
                                } else {
                                        if (in_array($m['id'], $rem_meals)) {
                        ...
                        $fmeals['snack1'][$i] = $snack1[0];
                        unset($micros_diff[$snack1[0]]);
                }
        }

        //$final_temp = filter_meal($meal, $days_npes_cal,$per_day_no_of_melas);
        //echo "<pre>8888";print_r($final_temp);
        //echo "<pre>-new-";print_r($meals);
        //echo "<pre>-new-";print_r($micros_diff);
        //echo "<pre>-new-";print_r($fmeals);
        $i++;
        if (!empty($micros_diff)) {
                $flipped = array_keys($micros_diff);

                $fmeals = select_balance_meals($flipped, $micros_array, $per_day_no_of_melas, $protin, $fat, $carbo, $i, $fmeals);
        }
...
function filter_meal($meal, $days_npes_cal, $per_day_no_of_melas = 4) {
        $final_temp = array();
        if (!empty($days_npes_cal) && !empty($meal)) {
                foreach ($days_npes_cal as $day => $v) {
                        if (in_array($meal['breakfast'], $v['breakfast']['meals'])) {
                                $final_temp[$day]['breakfast'] = $meal['breakfast'];
                        }
                        if (in_array($meal['lunch'], $v['lunch']['meals'])) {
                                $final_temp[$day]['lunch'] = $meal['lunch'];
                        }
                        if (in_array($meal['dinner'], $v['dinner']['meals'])) {
                                $final_temp[$day]['dinner'] = $meal['dinner'];
                        }

                        if ($per_day_no_of_melas == 5) {

                                if (in_array($meal['snack'], $v['pre_lunch_snack']['meals'])) {
                                        $final_temp[$day]['pre_lunch_snack'] = $meal['snack'];
                                } else if (in_array($meal['snack'], $v['pre_dinner_snack']['meals'])) {
                                        $final_temp[$day]['pre_dinner_snack'] = $meal['snack'];
                                }

                                if (in_array($meal['snack1'], $v['pre_lunch_snack']['meals'])) {
                                        $final_temp[$day]['pre_lunch_snack'] = $meal['snack1'];
                                } else if (in_array($meal['snack1'], $v['pre_dinner_snack']['meals'])) {
                                        $final_temp[$day]['pre_dinner_snack'] = $meal['snack1'];
                                }
                        } else {
                                if (in_array($meal['snack'], $v['pre_dinner_snack']['meals'])) {
                                        $final_temp[$day]['pre_dinner_snack'] = $meal['snack'];
                                }
                        }
                }
