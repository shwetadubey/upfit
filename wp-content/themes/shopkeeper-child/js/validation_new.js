...
//alert('test');
			...
			if(email===undefined || !expr.test(email) || jQuery.trim(email)===''){	 
				...
				jQuery('#list_email').tooltipster('update', err_msg[57]);
               ...
               // trigger MailChimp to return a JSONP response
                ...
                success: function(data){
					...
					if(data.result==='success'){
						jQuery('#mc_embed_signup').empty().append('<p class="mc_success">'+err_msg[41]+'</p>').hide().fadeIn(1500);
					}
					if(data.result==='error'){
						jQuery('#mc_embed_signup').empty().append('<p class="mc_error">'+err_msg[42]+'</p>').hide().fadeIn(1500);
					}
