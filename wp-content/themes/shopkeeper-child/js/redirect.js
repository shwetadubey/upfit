		...
		if(errors===false){
			...
			var user_data={	
					...
					//$.cookie('UpfitUserDataHome',JSON.stringify(user_data),{ expires: 1 })
						window.location.href=window.location.href.split('?')[0]+'ernaehrungsplan_erstellen/?current_weight='+cw+'&desired_weight='+dw;
					}
		}
...
//alert('test');
			...
			if(email===undefined || !expr.test(email) || jQuery.trim(email)===''){	 
				...
				jQuery('#list_email').tooltipster('update', error_msg[57]);
...
/*
jQuery('.popmake-close,#popmake-overlay').click(function(){
